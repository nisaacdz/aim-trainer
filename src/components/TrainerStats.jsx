...
const TrainerStats = ({ trainerState, restartSession, endSession }) => {
  ...
  return (
    <div className="w-full h-full mx-auto p-4 sm:p-6 bg-gray-100 shadow-lg">
      {/* Stats Container */}
      <div className="space-y-6">
        <h2 className="text-xl font-semibold text-center">Session Stats</h2>
        
        {/* Stats Grid */}
        <div className="grid gap-4">
          {/* Accuracy */}
          <div className="grid grid-cols-2 items-center gap-3">
            <label className="font-medium text-gray-700">Accuracy:</label>
            <span className="text-right">{trainerState.accuracy.toFixed(2)}%</span>
          </div>

          {/* Speed */}
          <div className="grid grid-cols-2 items-center gap-3" title="Average Response Time">
            <label className="font-medium text-gray-700">ART:</label>
            <span className="text-right">{Math.floor(trainerState.speed)}ms</span>
          </div>

          {/* Targets */}
          <div className="grid grid-cols-2 items-center gap-3">
            <label className="font-medium text-gray-700">Targets:</label>
            <span className="text-right">
              {trainerState.currentTargetCount} : {trainerState.totalTargets}
            </span>
          </div>

          {/* Time */}
          <div className="grid grid-cols-1 items-center gap-3">
            <CountUpTimer startTime={trainerState.startTime} isStopped={trainerState.endTime}/>
          </div>
        </div>
